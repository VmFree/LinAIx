...
LinAIx 提供系统级的 Agent 间通信和协调机制，但**不预设具体的协作模式**。这遵循操作系统设计的经典原则——**"机制与策略分离"**。

### 设计原则：机制与策略分离

| 层级 | 职责 | 类比 |
| :--- | :--- | :--- |
| **上层 (应用/框架)** | 决定"如何协作"（主从、流水线、投票、辩论...） | 应用层编排 |
| **LinAIx 内核** | 提供"协作的基础能力"（通信、同步、共享、调度...） | Linux 提供 pipe/socket/futex |

### LinAIx 提供的基础能力

| 能力 | 说明 | 类比 |
| :--- | :--- | :--- |
| **IPC 通道** | Agent 间标准化消息传递，支持点对点、广播、流式通信 | Linux `pipe()` / `socket()` |
| **共享内存** | Agent 间共享 KV Cache 和上下文数据，零拷贝高性能 | Linux `shm` |
| **同步原语** | Agent 间等待、通知、互斥锁，实现简单协调 | Linux `futex` / `semaphore` |
| **协作组 (Group)** | 多个 Agent 组成协作组，以组为单位进行资源配额和调度 | Linux `cgroup` |
| **组级调度** | L3 Scheduler 以组为单位分配 GPU 时间片，确保协同执行 | Linux `CFS` 组调度 |
| **全链路追踪** | 跨 Agent 请求通过统一 Trace ID 串联，完整可观测 | Linux `tracepoint` |
| **Agent 间授权** | Agent A 需明确权限才能向 Agent B 发送消息或调用其 Skill | Linux 文件权限 |
